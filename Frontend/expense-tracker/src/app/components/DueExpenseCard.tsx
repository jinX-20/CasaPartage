...
interface DueExpenseProps {
  ...
  userr: string;
  markAllAsPaid: (expenses: ExpenseDetail[], userr: string) => void; // Prop for markAllAsPaid
}

const DueExpenseCard: React.FC<DueExpenseProps> = ({ name, amount, expenses, userr, markAllAsPaid }) => {
  ...
  return (
    <div className="bg-[#EAE3CF] shadow-lg rounded-lg p-4 border border-gray-300">
      ...
      <div className="mt-3 flex space-x-4">
        <button
          onClick={() => markAllAsPaid(expenses, userr)} // Use the function passed from the parent
          className="bg-[#F5D28E] px-4 py-2 rounded text-black font-medium hover:bg-[#b19e77]"
        >
          Mark as paid!
        </button>

        <button className="bg-[#F5D28E] px-4 py-2 rounded text-black font-medium hover:bg-[#b19e77]">
          Pay Now!
        </button>
      </div>
    </div>
